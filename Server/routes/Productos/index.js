...
function buscarProducto(req, res, next) {
    ...
    cliente.connect();
    res.setHeader('Content-Type', 'application/json');
    query ="SELECT row_to_json(consulta) FROM (Select array_to_json(array_agg(productos)) as productos, 1 as valid from (SELECT SELECT idproducto, producto, codigo, imagen, descripcion, precioVenta, PrecioCompra, marca , m.idmarca, categoria, c.idcategoria, proveedor, pr.idproveedor, id.cantidad from marca m join marca_categoria mc using (idMarca) join categoria c using (idCategoria) join proveedor pr using (idProveedor) join producto using (idMarca) join precioProducto using (idProducto) join inventariodetalle id using(idproducto) join inventario i using(idinventario) where upper(p.producto) like upper('%"+req.params.busqueda+"%') or upper(m.marca) like upper('%"+req.params.busqueda+"%') or upper(c.categoria) like upper('%"+req.params.busqueda+"%') or upper(pr.proveedor) like upper('%"+req.params.busqueda+"%' and i.fecha = (SELECT fecha from inventario order by fecha desc limit 1)) as productos) as consulta";
    cliente.query(query).then(req => {
        const rows = req.rows;
        rows.map(row => {
            var respuesta = row.row_to_json;
