...
function buscarProducto(req, res, next) {
    ...
    cliente.connect();
    res.setHeader('Content-Type', 'application/json');
    query ="SELECT row_to_json(consulta) FROM (Select array_to_json(array_agg(productos)) as productos, 1 as valid from (SELECT p.idproducto, p.codigo, descripcion, pp.precioventa, pp.preciocompra, m.idmarca, c.idcategoria, pr.idproveedor,  p.producto, m.marca, pr.proveedor,c.categoria, p.imagen, id.cantidad FROM producto p join marca m using (idMarca) join marca_categoria using (idMarca) join categoria c using(idCategoria) join proveedor pr using(idProveedor) join precioproducto pp using(idproducto) join inventariodetalle id using(idproducto) join inventario i using(idinventario) where upper(p.producto) like upper('%"+req.params.busqueda+"%') or upper(m.marca) like upper('%"+req.params.busqueda+"%') or upper(c.categoria) like upper('%"+req.params.busqueda+"%') or upper(pr.proveedor) like upper('%"+req.params.busqueda+"%' and i.fecha = (SELECT fecha from inventario order by fecha desc limit 1)) as productos) as consulta";
    cliente.query(query).then(req => {
        const rows = req.rows;
        rows.map(row => {
            var respuesta = row.row_to_json;
...
function buscarProductoporID(req, res, next) {
    ...
    cliente.connect();
    res.setHeader('Content-Type', 'application/json');
    query ="SELECT row_to_json(consulta) FROM (Select array_to_json(array_agg(productos)) as productos, 1 as valid from (SELECT p.producto, m.marca, pr.proveedor,c.categoria, p.imagen, id. cantidad FROM producto p join marca m using (idMarca) join marca_categoria using (idMarca) join categoria c using(idCategoria) join proveedor pr using(idProveedor) join inventariodetalle id using(idproducto) join inventario i using(idinventario) where p.idproducto = "+req.params.id+" and i.fecha = (SELECT fecha from inventario order by fecha desc limit 1)) as productos) as consulta";
    cliente.query(query).then(req => {
        const rows = req.rows;
        rows.map(row => {
            var respuesta = row.row_to_json;
