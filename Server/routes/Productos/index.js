...
function buscarProducto(req, res, next) {
    ...
    cliente.connect();
    res.setHeader('Content-Type', 'application/json');
    query ="SELECT row_to_json(consulta) FROM (Select array_to_json(array_agg(productos)) as productos, 1 as valid from (SELECT p.producto, m.marca, pr.proveedor,c.categoria, p.imagen FROM producto p join marca m using (idMarca) join marca_categoria using (idMarca) join categoria c using(idCategoria) join proveedor pr using(idProveedor) where upper(p.producto) like upper('%"+req.params.busqueda+"%') or upper(m.marca) like upper('%"+req.params.busqueda+"%') or upper(c.categoria) like upper('%"+req.params.busqueda+"%') or upper(pr.proveedor) like upper('%"+req.params.busqueda+"%')) as productos) as consulta";
    cliente.query(query).then(req => {
        const rows = req.rows;
        rows.map(row => {
            var respuesta = row.row_to_json;
